...
void Xwindow::fillPolygon(int x, int y, int vertices, int side, int rotate, int colour){
	...
	std::unique_ptr<XPoint[]> point (new XPoint[vertices]);
//cout << "83 : made" << endl;
	point.get()[0] = XPoint{(short int)x, (short int)y};
//cout << "85 : accessed" << endl;
	double angle = rotate + pi/2;

	for ( int i = 1; i < vertices; ++i ){
		short int X = side * sin(angle);
		short int Y = side * cos(angle);
		point.get()[i] = XPoint{X,Y};
//cout <<"92: accessed" << endl;
		angle += 2*pi - pi* 2/vertices;
	}

	XSetForeground(d, gc, colours[colour]);
	XFillPolygon(d, w, gc, point.get(), vertices, Convex, CoordModePrevious);
//cout << "98: got" << endl;
	XSetForeground(d, gc, colours[Black]);
...
void Xwindow::drawString(int x, int y, string msg, int colour) {
  std::unique_ptr<XFontStruct> f (XLoadQueryFont(d, "6x13")) ;
//cout << "125 : made" << endl;
	printMessage(x, y, msg, colour, *(f.get())); 
//cout << "127 : got" << endl;
}


void Xwindow::drawStringFont(int x, int y, string msg, string font, int colour) {
  std::unique_ptr<XFontStruct> f (XLoadQueryFont(d, font.c_str()));
  
	if ( f.get() == nullptr ){
		f.reset(XLoadQueryFont(d, "6x13"));

//cout << "135 : reset" << endl;	

	}
if ( f.get() == nullptr ){cout<<"reset fail"<<endl;}
